...
import patientApi from '../api/patient';  // 添加这行
...
// 初始化加载数据
// 在 PatientDashboard.vue 的 onMounted 部分修改
onMounted(async () => {
...
const loadPatientInfo = async () => {
  try {
    console.log('调用患者信息接口...');
    const response = await patientApi.getPatientInfo();
    ...
    if (response.code === 200) {
      ...
      // 处理 null 值
      userEmail.value = userData.name || auth.userInfo?.email?.split('@')[0] || '用户';
      patientId.value = userData.id || 0;

      // 更新表单
      if (userData.name) {
        registrationForm.value.patientName = userData.name;
      }
      if (userData.phone) {
        registrationForm.value.patientPhone = userData.phone;
      }

      // 检查是否已完善信息
      if (!userData.name || !userData.phone) {
        ElMessage.warning({
          message: '请先完善个人信息以获得更好体验',
          duration: 3000
        });
      } else {
    ...
    // 如果404，可能是接口不存在，使用原来的 /auth/me
    if (error.response?.status === 404) {
      console.log('患者信息接口不存在，使用默认接口');
      // 可以调用原来的 /auth/me
    }
  }
};
